export const coreResults = {
  ...
  shopAccountNotFound: (id: string) => `쇼핑몰 계정 ${id} 을 찾지 못했다. list_shop_accounts 로 확인하라.`,
  shopNoStoredPassword: '이 쇼핑몰 계정에 저장된 비밀번호가 없다. 설정에서 넣어야 로그인할 수 있다.',
  shopNoAccounts: '등록된 쇼핑몰 계정이 없다. 설정의 Cafe24 쇼핑몰 계정 카드에서 추가하라고 안내할 것.',
  shopLoginDone: (detail: string) => detail,
  shopStepDone: (detail: string) => detail,
